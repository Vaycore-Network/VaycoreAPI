        ...
        fun onEvent(event: Event) {
            val items: List<ItemStack?> = supportedEvents.find { it.event.isAssignableFrom(event::class.java) }
                ?.handler?.let {
                    ...
                    Main.logger.warning("Tried to hook into ${event.javaClass.name} using ItemBuilder#onEvent. This event is not supported!")
                    return
                }
            ...
            for (item in items) {
                ...
                val id = meta.persistentDataContainer.get(
                    NamespacedKey("vaycore", "itembuilder"),
    ...
    fun <T : Event> onEvent(event: Class<T>, priority: EventPriority = EventPriority.NORMAL, handler: Consumer<T>): ItemBuilder {
        ...
        if (event !in registeredEvents) {
            ...
            val handlerList = try {
                ...
                Main.logger.warning("Error registering event handler for item $key: $e")
                return this
            }
